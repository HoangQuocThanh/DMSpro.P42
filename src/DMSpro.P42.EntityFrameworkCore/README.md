...
````bash
Add-Migration Your_Migration_Name -Context P42DbContext
````

Using EF Core command line tool;

````bash
dotnet ef migrations add Your_Migration_Name --context P42DbContext
````
...
````bash
Add-Migration Your_Migration_Name -Context P42TenantDbContext -OutputDir TenantMigrations
````

Using EF Core command line tool:

````bash
dotnet ef migrations add Your_Migration_Name --context P42TenantDbContext --output-dir TenantMigrations
````
